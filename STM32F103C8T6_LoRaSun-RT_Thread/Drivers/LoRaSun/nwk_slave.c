...
//      printf("###### NwkSlaveRxInit!\n");        
    ...
    {
      u8 sf=0, bw=0;
      pSlaveRx->freq=NWK_GW_BASE_FREQ+(g_sNwkSlaveWork.freq_ptr+g_sNwkSlaveWork.slave_addr)*2000000+pSlaveRx->group_id*500000;//计算监听频率
...
//    }
    ...
    case NwkSlaveTxAdrSniffInit:
    {
      ...
      nwk_slave_set_lora_param(freq, sf, bw);
			...
			for(u8 i=0; i<3+NWK_RF_GROUP_NUM-pSlaveTx->group_id; i++)//
			{
